...
const SimpleMiniChart = ({ variable }) => {
  ...
  }, [variable.id]); // Only depend on ID for stability

  // Calculate SVG path and metadata
  const { path, currentStatus, strokeColor } = useMemo(() => {
    if (!chartData || chartData.length === 0) {
      return { path: '', currentStatus: 'normal', strokeColor: '#00ff88' };
    }
    ...
    return { path: pathString, currentStatus: status, strokeColor: color };
  }, [chartData, variable.upperThreshold, variable.lowerThreshold, variable.id]);

...
console.log('📖 Generating variable definitions from CSV...');
...
try {
  ...
  // Build a map: variable name → definition
  ...
  dataLines.forEach((line, index) => {
    ...
    if (!variableName || !definition) {
      console.warn(`⚠️  Skipping line ${index + 2}: Missing variable name or definition`);
      return;
    }
  ...
  console.log(`📊 Found ${Object.keys(nameToDefinition).length} definitions in CSV`);
  ...
  while ((match = variablePattern.exec(processVariablesContent)) !== null) {
    ...
    if (nameToDefinition[name]) {
      ...
      console.warn(`⚠️  No definition found for variable ID ${id}: "${name}"`);
      unmatchedCount++;
  ...
  console.log(`✅ Matched ${matchCount} variables`);
  if (unmatchedCount > 0) {
    console.warn(`⚠️  ${unmatchedCount} variables without definitions:`);
  ...
  if (missingIds.length > 0) {
    console.error(`❌ Missing definitions for variable IDs: ${missingIds.join(', ')}`);
  }
...
`;
  ...
  console.log(`✅ Generated variableDefinitions.js with ${Object.keys(definitions).length} definitions`);
